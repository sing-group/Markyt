...
class DocumentsController extends AppController {
                        ...
                        foreach ($users as $id => $userName) {
                            ...
                            while ($index < $annotationsSize) {
                                ...
                                foreach ($annotations as $annotation) {
                                    ...
                                    $content .= $annotation['Annotation']['init'] . "\t" .
    ...
    function exportRawDocuments($projectId = null) {
        ...
        $this->response->download("documents.tsv");
        return $this->response;
    }

    public function reparseDocuments() {
        $total = $this->Document->find('count', array(
              'recursive' => -1, //int
        ));
        $buffer = 1000;
        ...
        for ($i = 0; $i < $total; $i += $buffer) {
            $documents = $this->Document->find('all', array(
                  'recursive' => -1, //int
                  'fields' => array('Document.id', 'Document.external_id', 'Document.html'),
                  'limit' => $buffer, //int
                  'page' => $cont, //int
            ));
            foreach ($documents as $key => $document) {
                $sections = explode("</h3>", $document["Document"]["html"]);
                if (count($sections) > 1) {
                    $title = $this->parseHtmlToGetAnnotations($sections[0] . "</h3></div>");
                    $abstract = $sections[1];
                    $documents[$key]["Document"]["title"] = $title;
                    $documents[$key]["Document"]["raw"] = $title . "\t" . $abstract;
                }
            }
            $this->Document->saveMany($documents);
            $cont++;
        }
    }
                    ...
                    if ($file->readable()) {
                        ...
                        if (!empty($lines)) {
                            ...
                            for ($index = 0; $index < $size; $index++) {
                                if ($index == 0 && strpos(strtoupper($lines[$index]), "ABSTRACT") == false) {
                                    continue;
                                }
                                if (strlen($lines[$index]) > 1) {
                                    $section = explode("\t", trim($lines[$index]));
                                    if (sizeof($section) == 3) {
                                        ...
                                        $documentInDb = $this->Document->find('first', array(
                                              'recursive' => -1, //int
                                        ...
                                        if (empty($documentInDb)) {
                                            $this->Document->create();
                                            if (!$this->Document->save($data)) {
                                                debug($data);
                                                debug($this->Document->invalidFields());
                                                $db->rollback();
                                                $index++;
