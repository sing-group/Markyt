...
<div class="col-xs-11" id="documentBodyContainer">
    ...
    if (count($typeIds) > 1) {
        ...
        $typesCols = array();
        ?>
        <div class="alert alert-warning"><span class="fa fa-info-circle"></span> There is not enough types top create a confrontation table  </div>
        <?php
    }
    echo $this->element('annotatedDocumentWithRelationalTable', array("typesRows" => $typesRows,
          "typesCols" => $typesCols));
    ?>
</div>
