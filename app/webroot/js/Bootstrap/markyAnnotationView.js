...
function annotationEditRemove(evt) {
    ...
    if (evt.button === 0) {
        ...
        dialogForm.find(".modal-body").empty();

        idForm = "#" + type + "Form";
        questioIds = $(idForm).attr('value');


        containerDiv = dialogForm.find(".modal-body");
        ...
        if (questioIds) {
            ...
            jQuery.post(MarkyLocation + '/annotationsQuestions/ajaxEdit/', {mode: "view", annotation_id: valueThis, questions: arrayIds, user_round_id: user_round_id, round_id: round_id, user_id: user_id, document_id: document_id}, function (data) {
                if (!data.success) {
                    ...
                    if (data.annotationsQuestion.length > 0) //este caso sucede siempre que se hace un request de una anotacion que de un tipo sin preguntas
                        annotationsQuestions = data.annotationsQuestion;
                    ...
                    containerForm = $(idForm).clone();
                    containerDiv.append(containerForm);
                    arrayToForm($("#dialog-form").find("form").removeClass("hidden").get(0), annotationsQuestions);
                ...
                swal({
                    title: "There is an error in the communication with the server",
                    type: "info",
                    closeOnConfirm: true,
                });
            });
            ...
            containerDiv.append('<p><span class="ui-icon ui-icon-info"></span><span class="bold ">No answers for this annotation</span></p>');
            dialogForm.modal({
            });
        }
    }
...
function documentAssessment() {
    ...
    $("#assessmentButton").click(function () {
        ...
        $.ajax({
            ...
            modal.find(".label").addClass("hidden");
            ...
            if (!$.isEmptyObject(result)) {
                ...
                modal.find(".about_author").text(result.DocumentsAssessment.about_author);
                modal.find(".topic").text(result.DocumentsAssessment.topic);
                modal.find(".note").text(result.DocumentsAssessment.note);
            ...
            {
                modal.find('.document-info').addClass("hidden")
                modal.find(".label-default").removeClass("hidden");
                modal.find(".about_author").text('');
                modal.find(".topic").text('');


                modal.find(".note").text('');
            }
